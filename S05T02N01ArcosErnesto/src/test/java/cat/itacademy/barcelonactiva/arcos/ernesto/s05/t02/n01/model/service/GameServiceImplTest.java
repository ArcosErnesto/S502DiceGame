...
class GameServiceImplTest {
    ...
    void getOnePlayerGames_should_return_list_of_games_for_existing_player() {
        ...
        List<GameEntity> result = gameService.getOnePlayerGames(player1.getPlayerId());
        assertEquals(2, result.size());
        assertEquals(player1.getPlayerId(), result.get(0).getPlayerEntity().getPlayerId());
        assertEquals(player1.getPlayerId(), result.get(1).getPlayerEntity().getPlayerId());
    }
    ...
    void deletePlayerGames_should_delete_games_for_existing_player() {
        ...
        List<GameEntity> games = Arrays.asList(game1, game2);
        when(gameRepository.findAll()).thenReturn(games);
        String result = gameService.deletePlayerGames(player1.getPlayerId());
